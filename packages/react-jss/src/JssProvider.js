// @flow
import React, {Component, Children, type Element, type ElementType, createContext} from 'react'
...
export default class JssProvider extends Component<Props> {
  ...
  render() {
    ...
    return (
      <Consumer>
        {value => {
          const sheetOptions = value[ns.sheetOptions] || {}
          const newContext: Context = {[ns.sheetOptions]: sheetOptions}

          if (registry) {
            newContext[ns.sheetsRegistry] = registry
          ...
          newContext[ns.managers] = this.managers
          ...
          if (localJss) newContext[ns.jss] = localJss
          if (disableStylesGeneration !== undefined) {
            sheetOptions.disableStylesGeneration = disableStylesGeneration
          }

          return <Provider value={newContext}>{Children.only(this.props.children)}</Provider>
        }}
      </Consumer>
    )
  }
